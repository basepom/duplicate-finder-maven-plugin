...
The <tt>maven-duplicate-class-finder-plugin</tt> is a plugin that will search for classes with the same name, as well as resources with the same path,
...
<tt>com.ning.maven.plugins</tt> group to the <tt>pluginGroups</tt> section in your settings file:
    ...
    <settings>
      ...
      <pluginGroups>
        <pluginGroup>ning.maven.plugins</pluginGroup>
      </pluginGroups>
      ...
    </settings>
...
You can also run a specific version using the fully qualified plugin identifier:
    ...
    mvn com.ning.maven.plugins:maven-class-finder-plugin:1.0.3:check
