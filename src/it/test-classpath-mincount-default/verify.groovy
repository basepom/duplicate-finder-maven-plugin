...
assert "2" == xml.preferences.@resultFileMinClasspathCount.text()
...
assert 2 == resources.classpathElement.size() // conflict-same-content, conflict-different-content

def classes = result.classpathElements.findAll({ it.@type.text().equals(TYPE_CLASS) })
assert null != classes
assert 1 == classes.classpathElement.size() // diff.Demo
